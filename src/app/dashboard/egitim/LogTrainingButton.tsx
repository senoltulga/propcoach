...
export default function LogTrainingButton({ agents }: { agents: Agent[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setOpen(false)
        setSuccess(false)
        setForm({ agent_id: '', title: '', category: 'genel', training_date: '', score: '', notes: '' })
  ...
  return (
    <>
      <button
        ...
        className="px-4 py-2 border border-emerald-600 text-emerald-700 text-sm rounded-lg hover:bg-emerald-50 transition-colors"
      >
        + Eğitim Kaydı
      </button>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="font-semibold text-gray-900">Eğitim Kaydı Ekle</h2>
